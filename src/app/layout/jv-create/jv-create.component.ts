...
@Component({
  ...
  templateUrl: './jv-create.component.html',
  styleUrls: ['./jv-create.component.scss']
})
export class JvCreateComponent implements OnInit {
  ...
  searchAccount(searchText) {
    this.jvCreateService.searchAccount('JV' + searchText).then((data) => {
      if (data._id) {
